...
calculateCustomBeta(){
  const data = {
    value:'customBeta',
    betaValue: this.excessEarningForm.controls['beta'].value
  }
  ...
  betaDialogPrev.afterClosed().subscribe((result)=>{
    if (result) {
      this.excessEarningForm.controls['beta'].setValue(convertToNumberOrZero(result?.customBeta))
...
calculateStockBeta(){
  ...
  loadBetaDropdown() {
    const betaDropdownValues = this.modelControl.excessEarning.options.betaType.options.slice().filter((options:any) => options.enabled === true);
